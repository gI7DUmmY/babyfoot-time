...
const DateEvents = ({ events }: Props) => {
  return events.length === 0 ? (
    ...
    // DEBUT periode event temporairement en dur
    <>
      <ul className='flex flex-col items-center text-center gap-4 md:items-start md:text-left'>
        <li>
          Du 01/07 au 17/07 19:30 <span className='hidden md:inline'>:</span>
          <br className='md:hidden' /> La Gare de la Magdelaine
          <br className='md:hidden' />
          <Link
            href='https://tally.so/r/jaga5a'
            target='_blank'
      ...
      {/*FIN periode event temporairement en dur*/}
